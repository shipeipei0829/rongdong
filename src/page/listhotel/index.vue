<template>
  <div class="listhotel">
      <div class="filter">
        <div class="inputwap clearfix">
          <div class="inputleft fl" @click="handleRouter('left')"> <van-icon name="arrow-left" />  </div>
          <van-search class="inputright fl" placeholder="请输入搜索关键词" v-model="checkvalue" >
            <div class="ipcon" slot="label" @click="onSearch"><span>北京</span></div>
          </van-search>
        ...
        <div class="timewap">
          <div class="timecon">
            <p @click="handledate('left')">住：{{startDate.split('-')[1]}} 月 {{startDate.split('-')[2]}}日 {{startTime == '' ? '12:00':startTime}}</p>
            <p @click="handledate('right')">离：{{endDate.split('-')[1]}} 月 {{endDate.split('-')[2]}}日 {{startTime == '' ? '12:00':endTime}}</p>
          </div>
        </div>

        <div v-if="thislistDateMore.length>0 && handlestopCloseShow " class="filterconwap">
          <div class="filtercon">
            <span @click="handleRouter('ipcon',{})"> <i>{{filterInfo.ipcon.name}}</i>  <van-icon name="arrow-down" /> </span>
            <span @click="handleRouter('rmbfilter',{})"> <i>{{showrmb({startrmb:filterInfo.sRoomPrice,endrmb:filterInfo.roomPrice}) }}</i> <van-icon name="arrow-down" /> </span>
            <span @click="handleRouter('morefilter',{})"> <i>更多筛选</i> <van-icon name="arrow-down" /> </span>
            <span class="popconwapR" @click="handleRouter('sort',{})"> <i>排序</i>   <van-icon name="arrow-down" /> 
            </span>
          </div>

          <div class="popconwap">
          <div class="text">
            默认排序精准入住时间匹配度最高 您可通过筛选条件排序更多酒店
          </div>
                <Collectitem   v-for="(x,index) in thislistDateMore" :dateobj="x" :key="index" class="itemwaps"  />
          
          <div @click="handlestopClose()" class="btntext">
            我知道了
          </div>
          </div>
        </div>
...
export default {
  ...
  data(){
    return{
      ...
      handlestopCloseShow:true,//提示弹框 是否显示出来
      changeVal:'',
      startrmb:'',
      endrmb:'',
  ...
  methods:{
    ...
    handlestopClose(){
      localStorage.setItem('handlestopCloseShow',false)
      this.handlestopCloseShow = false
    },
    loadlistup(){
    ...
    var t = this
      ...
      console.log(data)
      if(this.pageType == 'filterHotel'){
         this.setloadlist(data)
      }else if(this.pageType == 'sell'){
         this.setloadlistsell(data)
      }
     

      
    },
    setloadlistsell(data){
      var t = this
      loadlistprice(
        data
      ).then(res => {
        console.log(res)
        if(res.code == 0){
          if(Math.ceil(res.data.total/res.data.size) ==  res.data.current){
        ...
        if(res.code == 0){
          if(res.data.records.length > 0){
            ...
            if(t.thispage == true){//第一次加载页面
              ...
              t.thislistDateMore.push(t.listDateMore[0])
            }else{
              t.thislistDateMore.push(t.listDateMore[0])
  ...
  mounted(){
    ...
    if(localStorage.getItem('handlestopCloseShow') == null){
      this.handlestopCloseShow = true
    }
    if(localStorage.getItem('handlestopCloseShow') == 'false'){
      this.handlestopCloseShow = false
    }
...
.popconwap{
  ...
  box-shadow: 0px 0px .1rem .1rem rgba(0, 0, 0, 0.2);
  .text{
    font-size: .38rem;
    color: #FF9624;
    width: 5.94rem;
